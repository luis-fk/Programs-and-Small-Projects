...
int main()
{
    /* distance of the first body from the origin on the "x" and "y" axis, rx0 and ry0
       respectively, as well as its velocy on the "x" and "y" axis, vx0 and vy0 respectively,
       and its mass m0. */
    double rx0, ry0, vx0, vy0, m0;
    /* distance of the second body from the origin on the "x" and "y" axis, rx0 and ry0
       respectively, as well as its velocy on the "x" and "y" axis, vx0 and vy0 respectively,
       and its mass m0. */
    double rx1, ry1, vx1, vy1, m1;
    /* distance of the third body from the origin on the "x" and "y" axis, rx0 and ry0
       respectively, as well as its velocy on the "x" and "y" axis, vx0 and vy0 respectively,
       and its mass m0. */
    double rx2, ry2, vx2, vy2, m2;
    /* dT denotes the time interval between each calculation, so it can be seen as the
       update interval. T is the total amount of time for which the calculations should
       run */
    double dt, T;
